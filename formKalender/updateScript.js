...
document.getElementsByTagName("form")[0].addEventListener("submit", (e) =>{
    ...
    var today = new Date(),
    endDateComp = new Date(endDate.value);
    
    if(endDate.value == ""){
        warning.innerHTML += "Please input END DATE<br>";
        endDate.setAttribute("class", "warningBox");
    } else if (endDateComp < today) {
        warning.innerHTML += "PASSED EVENT can NOT be update<br>";
        endDate.setAttribute("class", "warningBox");
...
function sendData() {
    ...
    xhr.open("POST", "updateValid.php?" + params, true);
    xhr.send();
